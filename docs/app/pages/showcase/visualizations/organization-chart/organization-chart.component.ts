...
export class OrganizationChartShowcaseComponent implements OnInit, OnDestroy {
  ...
  dataset: OrganizationChartNode<OrganizationChartContext> =
    this._dataService.getDataset().children[0];
  ...
  onTypeaheadSelect(
    event: TypeaheadOptionEvent | OrganizationChartNode<OrganizationChartContext>,
    focusOrigin: FocusOrigin = 'mouse'
  ): void {
  ...
  search(): void {
    ...
    this.typeaheadOptions = options.filter(option => {
      return (
        option.data.name.toLowerCase().indexOf(this.query.toLowerCase()) !== -1 ||

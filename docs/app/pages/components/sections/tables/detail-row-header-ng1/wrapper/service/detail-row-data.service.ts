detailRowDataService.$inject = ['$rootScope', '$colorService'];

function detailRowDataService($rootScope: angular.IRootScopeService, $colorService: any) {
    ...
    function detailRowSortByKey(array: any[], key: string, ascending: boolean[]) {
    ...
    function applyFilters(filters: any[]) {
        ...
        if (filters.length !== 0) {
            ...
            detailRowTable = detailRowOriginal.filter(row => {
                return filters.findIndex(filter => filter.address === row.address) !== -1;
            });
        } else {
    ...
    function getDetailRowTable(sorter: any, ascending: any) {
        detailRowSortByKey(detailRowTable, sorter, ascending);
        return detailRowTable;
    }
}

angular.module('app').service('detailRowDataService', detailRowDataService);

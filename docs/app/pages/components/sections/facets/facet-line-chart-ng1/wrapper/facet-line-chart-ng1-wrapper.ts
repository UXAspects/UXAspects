angular.module('app').controller('LineChartFacetCtrl', LineChartFacetCtrl);

angular.module('app').directive('uxdFacetLineChartWrapper', () => {
    return {
        restrict: 'E',
        controller: 'LineChartFacetCtrl as lc',
        template: require('./facet-line-chart-ng1-wrapper.html')
    };
});

LineChartFacetCtrl.$inject = ['$scope'];

function LineChartFacetCtrl($scope: angular.IScope) {
    var lc = this;
    ...
    */
    ...
    lc.selectDateRange = function () {
        ...
        if (lc.dateStart.getTime() > lc.dateEnd.getTime()) {
            lc.invalidDate = true;
        } else {
            lc.invalidDate = false;
        }
    ...
    function updateChart(startDate: Date, endDate: Date) {
    ...
    */
    ...
    lc.dateStartOpen = function ($event: Event) {
    ...
    lc.dateStartUpdate = function () {
        ...
        if (lc.dateStart.getTime() < minimumDate) {
            lc.dateStart = new Date(minimumDate);
        }

        if (lc.dateStart.getTime() > maximumDate) {
            lc.dateStart = new Date(maximumDate);
        }
    ...
    */
    ...
    lc.dateEndOpen = function ($event: Event) {
        // show date picker and make sure other date picker is hidden
        lc.dateStartIsOpen = false;
    ...
    lc.dateEndUpdate = function () {
        ...
        if (lc.dateEnd.getTime() < minimumDate) {
            lc.dateEnd = new Date(minimumDate);
        }

        if (lc.dateEnd.getTime() > maximumDate) {
            lc.dateEnd = new Date(maximumDate);
        }
    ...
    lc.lineChart = {
        ...
        lineoptions: {
            series: {
                ...
                shadowSize: 0,
                highlightColor: [flotChartColors.secondary1]
            },
            xaxis: {
                tickDecimals: 0,
                mode: 'time',
    ...
    */
    function getPointsWithinRange(rangeStart: number, rangeEnd: number, increment: number, low: number, high: number) {
        var points = [];

...
const config = {
  ...
  beforeLaunch() {
    ...
    execSync('nx build ux-aspects-e2e', { stdio: 'inherit' });
...
if (!isJenkinsBuild) {
  ...
  config.plugins.find(
    plugin => plugin.package === 'protractor-image-comparison'
  ).options.autoSaveBaseline = true;
}
...
function getHostAddressFromSeleniumContainer() {
  ...
  if (!isIp(address)) {
    throw new Error(
      `Expected an IP address but got "${address}". Make sure docker container ${DOCKER_CONTAINER_NAME} is running.`
    );
  }

...
import { SelectPage } from './select.po.spec';
...
describe('Select Tests', () => {
  ...
  it('should expand customization panel', () => {
      
    page.clickOnPanel();
    expect(page.confirmPanelIsExpanded()).toBeTruthy();
  ...
  it('should use the paging size', () => {
    ...
    page.clickOnDropdown(false);
    expect<any>(page.getNumberOfCountries(false)).toEqual(SelectPage.numberOfCountries);
  ...
  it('should allow text entry of the paging size', () => {
    ...
    // change page size by entering a number
    page.clickOnPageSize();
    page.getPageSize().clear();
    page.getPageSize().sendKeys(Key.ARROW_LEFT);
    page.getPageSize().sendKeys('8' + Key.ENTER); // the min number is 1 so after clear(), 1 is already there
    var pageSize = 81;
    
    // confirm number of visible countries increases by 81 each time
    expect(page.waitForLoadingAfterClickToFinish(false)).toBeTruthy();
    result = page.calculateNewNumberOfCountries(false, pageSize);    
    expect(page.waitForLoadingAfterHoverToFinish(false)).toBeTruthy();
    expect<any>(page.getNumberOfCountries(false)).toEqual(result);

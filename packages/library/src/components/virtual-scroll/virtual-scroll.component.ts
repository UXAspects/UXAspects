...
export class VirtualScrollComponent
    ...
    @ContentChild(VirtualScrollCellDirective, { read: TemplateRef })
    cellTemplate: TemplateRef<any>;
    @ContentChild(VirtualScrollLoadingDirective, { read: TemplateRef })
    loadingIndicatorTemplate: TemplateRef<any>;
    @ContentChild(VirtualScrollLoadButtonDirective, { read: TemplateRef })
    ...
    constructor(private _elementRef: ElementRef, resizeService: ResizeService) {
        ...
        resizeService
            .addResizeListener(_elementRef.nativeElement)
    ...
    ngOnInit() {
        if (!this.cellHeight) {
            throw new Error(
                'Virtual Scroll Component requires "cellHeight" property to be defined.'
            );
        }
    ...
    renderCells(): void {
        ...
        if (this.loadOnScroll && !this.isLoading && !this.loadingComplete) {
            ...
            if (
                remainingScroll <= this._elementRef.nativeElement.clientHeight
            ) {
                this.loadNextPage();
            }
        }

...
describe('Sliders', () => {
  ...
  it('should display the expected drag handles', async () => {
    ...
    expect(
      await page.getHandleAttribute(page.singleValueCustomLabels, 'lower', 'hidden')
    ).toBeNull();
    expect(
      await page.getHandleAttribute(page.singleValueCustomLabels, 'upper', 'hidden')
    ).not.toBeNull();

    expect(
      await page.getHandleAttribute(page.singleValueCalloutOnDrag, 'lower', 'hidden')
    ).toBeNull();
    expect(
      await page.getHandleAttribute(page.singleValueCalloutOnDrag, 'upper', 'hidden')
    ).not.toBeNull();

    expect(
      await page.getHandleAttribute(page.singleValueNarrowSnapping, 'lower', 'hidden')
    ).toBeNull();
    expect(
      await page.getHandleAttribute(page.singleValueNarrowSnapping, 'upper', 'hidden')
    ).not.toBeNull();

    expect(
      await page.getHandleAttribute(page.singleValueNarrowCalloutOnHover, 'lower', 'hidden')
    ).toBeNull();
    expect(
      await page.getHandleAttribute(page.singleValueNarrowCalloutOnHover, 'upper', 'hidden')
    ).not.toBeNull();
  ...
  it('should display the expected ticks', async () => {
    ...
    expect(await page.getTickAttribute(page.singleValueCustomLabels, 'class', 0)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueCustomLabels, 'class', 1)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueCustomLabels, 'class', 2)).toContain(
      'major'
    );
    ...
    expect(await page.getTickAttribute(page.singleValueCalloutOnDrag, 'class', 0)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueCalloutOnDrag, 'class', 2)).toContain(
      'minor'
    );
    expect(await page.getTickAttribute(page.singleValueCalloutOnDrag, 'class', 4)).toContain(
      'major'
    );
    ...
    expect(await page.getTickAttribute(page.singleValueNarrowSnapping, 'class', 0)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueNarrowSnapping, 'class', 3)).toContain(
      'minor'
    );
    expect(await page.getTickAttribute(page.singleValueNarrowSnapping, 'class', 5)).toContain(
      'major'
    );

    // Narrow - Gradient Track Styling - Callout on Hover.
    expect(
      await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'hidden', 0)
    ).toBeNull();
    expect(
      await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'hidden', 5)
    ).toBeNull();
    expect(
      await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'hidden', 7)
    ).toBeNull();
    expect(
      await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'hidden', 10)
    ).toBeNull();
    ...
    expect(await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'class', 0)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'class', 5)).toContain(
      'major'
    );
    expect(await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'class', 7)).toContain(
      'minor'
    );
    expect(
      await page.getTickAttribute(page.singleValueNarrowCalloutOnHover, 'class', 10)
    ).toContain('major');
  ...
  it('should have correct initial values', async () => {
    ...
    expect(await page.getHandleAttribute(page.singleValueCustomLabels, 'lower', 'style')).toContain(
      'left: 50%'
    );
    ...
    expect(
      await page.getHandleAttribute(page.singleValueNarrowSnapping, 'lower', 'style')
    ).toContain('left: 80%');
  ...
  it('should react to drag and drops', async () => {
    ...
    await page.dragAndDropHandle(page.singleValueNarrowCalloutOnHover, 'lower', { x: 2000, y: 0 });
    await page.mouseDownOnHandle(page.singleValueNarrowCalloutOnHover, 'lower');
    expect(await page.getTooltipValue(page.singleValueNarrowCalloutOnHover, 'lower')).toEqual(
      '100'
    );
  ...
  it('should never allow negative ranges', async () => {
    ...
    await page.dragAndDropHandle(page.rangeCalloutCustom, 'lower', { x: 2000, y: 0 });
    expect(await page.getTooltipValue(page.rangeCalloutCustom, 'lower')).toEqual(
      await page.getTooltipValue(page.rangeCalloutCustom, 'upper')
    );

    expect(await page.getSliderRangeAttribute(page.rangeCalloutCustom, 'style')).toContain(
      'flex-grow: 0;'
    );
  });
  ...
  describe('in disabled state', () => {
    ...
    it('should not react to drag and drop', async () => {
      await page.dragAndDropHandle(page.rangeWithTextInputs, 'lower', { x: -2000, y: 0 });
      expect(await page.getHandleAttribute(page.rangeWithTextInputs, 'lower', 'style')).toContain(
        'left: 25%'
      );
    ...
    it('should not react to drag and drop (range)', async () => {
      ...
      await page.dragAndDropHandle(page.rangeWithTextInputs, 'upper', { x: 2000, y: 0 });
      expect(await page.getHandleAttribute(page.rangeWithTextInputs, 'upper', 'style')).toContain(
        'left: 75%'
      );

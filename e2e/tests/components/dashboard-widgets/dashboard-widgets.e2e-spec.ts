...
describe('Dashboard Widgets Tests', () => {
    ...
    it('should have correct initial states', async () => {
        ...
        expect(await page.getWidgetLocationValue(widgetEnum, 'top')).toBe(440);
        expect(await page.getWidgetLocationValue(widgetEnum, 'left')).toBe(0);

        expect(await page.getWidgetLocationValue(widgetTable, 'top')).toBe(440);
        expect(await page.getWidgetLocationValue(widgetTable, 'left')).toBe(554);

        expect(await page.getWidgetLocationValue(widgetText, 'top')).toBe(440);
        expect(await page.getWidgetLocationValue(widgetText, 'left')).toBe(831);
    ...
    it('should react correctly when a widget is moved down', async () => {
        ...
        const layoutMock =  [{ id: 'widget-actions', col: 0, row: 1, colSpan: 4, rowSpan: 2},
                             { id: 'widget-enum', col: 0, row: 0, colSpan: 2, rowSpan: 1},
                             { id: 'widget-table', col: 2, row: 0, colSpan: 1, rowSpan: 1},
                             { id: 'widget-text', col: 3, row: 0, colSpan: 1, rowSpan: 1}];

        // drag the top widget down
        await browser.actions().dragAndDrop(widgetActions, { x: 0, y: 250 }).perform();

        expect(await page.getWidgetLocationValue(widgetActions, 'top')).toBe(220, 'widget1 top');
        expect(await page.getWidgetLocationValue(widgetActions, 'left')).toBe(0, 'widget1 left');

        expect(await page.getWidgetLocationValue(widgetEnum, 'top')).toBe(0, 'widget2 top');
        expect(await page.getWidgetLocationValue(widgetEnum, 'left')).toBe(0, 'widget2 left');

        expect(await page.getWidgetLocationValue(widgetTable, 'top')).toBe(0, 'widget3 top');
        expect(await page.getWidgetLocationValue(widgetTable, 'left')).toBe(554, 'widget3 left');

        expect(await page.getWidgetLocationValue(widgetText, 'top')).toBe(0, 'widget4 top');
        expect(await page.getWidgetLocationValue(widgetText, 'left')).toBe(831, 'widget4 left');
    ...
    it('should allow subsequent widgets to be moved', async () => {
        ...
        await browser.actions().dragAndDrop(widgetEnum, { x: 500, y: 250 }).perform();

        expect(await page.getWidgetLocationValue(widgetActions, 'top')).toBe(440, 'widget1 top');
        expect(await page.getWidgetLocationValue(widgetActions, 'left')).toBe(0, 'widget1 left');

        expect(await page.getWidgetLocationValue(widgetEnum, 'top')).toBe(220, 'widget2 top');
        expect(await page.getWidgetLocationValue(widgetEnum, 'left')).toBe(554, 'widget2 left');

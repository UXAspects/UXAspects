...
describe('Toolbar Search', () => {
  ...
  it('should display search input when the button is clicked', async () => {
    ...
    // Verify input focus
    expect(await browser.driver.switchTo().activeElement().getId()).toEqual(
      await page.leftInput.getId()
    );
  ...
  describe('always expanded', () => {
    ...
    it('should not allow text to overflow behind buttons on toolbar search component', async () => {
      ...
      expect(await page.leftInput.getAttribute('class')).toContain(
      ...
      // Enter search text
      await page.rightInput.sendKeys(
        'mmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmmm'
      );

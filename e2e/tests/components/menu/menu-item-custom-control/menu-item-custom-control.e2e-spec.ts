...
describe('Menu Item Custom Control Directive', () => {
  ...
  describe('When menu open', () => {
    beforeEach(async () => {
      ...
      // open menu
      await browser.actions().sendKeys(Key.TAB).sendKeys(Key.SPACE).perform();
    });

    it('should focus the component input and display the focus indicator', async () => {
      expect(await page.activeElementAttr('id')).toBe('test-toggleswitch-input');
    ...
    it('should focus the inputs of the components when moving through the menu with keyboard navigation', async () => {
      await browser.actions().sendKeys(Key.ARROW_DOWN).sendKeys(Key.ARROW_DOWN).perform();
    ...
    it('should focus custom elements', async () => {
      await browser.actions().sendKeys(Key.ARROW_UP).perform();
      expect(await page.activeElementAttr('id')).toBe('password-input');
    });
  });
  ...
  it('should update the toggleswitch value using the keyboard', async () => {
    ...
    await browser.actions().sendKeys(Key.TAB).sendKeys(Key.SPACE).sendKeys(Key.SPACE).perform();
  ...
  it('should update the checkbox value using the keyboard', async () => {
    expect(await page.checkboxValue.getAttribute('innerText')).toBe(

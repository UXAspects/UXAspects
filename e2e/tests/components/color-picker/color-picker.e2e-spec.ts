...
describe('Color Picker', () => {
    ...
    it('should focus the first color when accessing via the keyboard', async () => {
        ...
        expect(await browser.driver.switchTo().activeElement().getAttribute('aria-label')).toEqual('Select color Primary');
    ...
    it('should focus the most recently selected color when accessing via the keyboard', async () => {
        ...
        expect(await browser.driver.switchTo().activeElement().getAttribute('id')).toEqual('color-picker-toggle');

        // Reopen the dropdown
        await page.colorPickerToggle.sendKeys(Key.ENTER);
        expect(await browser.driver.switchTo().activeElement().getAttribute('aria-label')).toEqual('Select color Secondary');
    });
});

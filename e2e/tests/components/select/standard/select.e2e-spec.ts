...
describe('Select Tests', () => {
  ...
  it('should react to use of the "multiple" checkbox', async () => {
    ...
    expect(await page.getSelectedLocationText()).toBe(
      '[ "Antarctica", "Bangladesh", "Bosnia and Herzegovina" ]'
    );
    ...
    expect(await page.getSelectedLocationText()).toBe(
      '[ { "id": 40, "name": "Cameroon" }, { "id": 50, "name": "Colombia" } ]'
    );
  ...
  it('should be possible to remove tags', async () => {
    ...
    expect(await page.getSelectedLocationText()).toBe(
      '[ "French Southern Territories", "Guadeloupe", "Israel" ]'
    );

    // remove 2nd tag
    await page.removeCountry(1);
    expect(await page.getSelectedLocationText()).toBe(
      '[ "French Southern Territories", "Israel" ]'
    );
  ...
  it('should allow quick filtering when using a lower filterDebounceTime value', async () => {
    ...
    expect(await page.getSelectedLocationText()).toBe(
      '"United States"',
      'should select incorrect value when debounceTime is 1200'
    );
    ...
    expect(await page.getSelectedLocationText()).toBe(
      '"Aland Islands"',
      'should select correct value when debounceTime is 0'
    );
  });
  ...
  it('should automatically change the drop direction depending on the available space when using dropDirection=auto', async () => {
    ...
    await page.clickOnDropdown(false);
    expect(await page.confirmDropdownIsExpanded()).toBe(true);
    expect(await imageCompare('select-drop-direction-auto-down')).toBe(
      0,
      'should use direction down when space is available'
    );

    await page.increaseMaxHeight.click();
    await page.clickOnDropdown(false);
    expect(await page.confirmDropdownIsExpanded()).toBe(true);
    expect(await imageCompare('select-drop-direction-auto-up')).toBe(
      0,
      'should use direction up when space is not available'
    );
  });

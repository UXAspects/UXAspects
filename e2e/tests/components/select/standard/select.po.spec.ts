...
export class SelectPage {
  ...
  checkSelectedOptionsButton() {
    return this.radioOptions
      .$('ux-radio-button[option="strings"]')
  ...
  checkSelectedDirectionButton() {
    return this.radioDirection
      .$('ux-radio-button[option="down"]')
      .$('.ux-radio-button-checked')
      .isPresent();
  }
  ...
  confirmPageSizeButtonIsDisabled(direction: string) {
    if (direction === 'down') {
      return this.confirmClassExists(
        this.pageSize.$('div.number-picker-controls').$$('div').get(0),
        'disabled'
      );
    } else {
      return this.confirmClassExists(
        this.pageSize.$('div.number-picker-controls').$$('div').get(1),
        'disabled'
      );
    }
  }
  ...
  getTag(index: number) {
    return this.dropdown
      .$('ux-tag-input')
      .$$(this.dropdown.$('ux-tag-input').locator().value + ' > ol')
      .get(0)
      .$$('li')
      .get(index);
  }
  ...
  getFilterText(index: number) {
    return this.getCountry(false, index)
      .$('span.ux-typeahead-option')
      .$('span.ux-filter-match')
      .getText();
  }
  ...
  async clickOnIncrementFilterDebounce() {
    await $('#filterDebounceTimeInput div.number-picker-controls')
  ...
  hoverOverLastCountry(allowMultiple: boolean) {
    if (allowMultiple) {
      ...
      browser
        .actions()
        .mouseMove(
          this.dropdown.$('ux-typeahead').$('div.ux-typeahead-options').$('ol').$$('li').last()
        )
        .perform();
    }
  }
  ...
  calculateNewNumberOfCountries(allowMultiple: boolean, pageSize: number) {
    return this.getNumberOfCountries(allowMultiple).then(function (count) {
      if (count > numberOfCountries - pageSize) {
        // makes sure number can't exceed 251
  ...
  waitForLoadingToFinish() {
    ...
    const elem: ElementFinder = this.dropdown
      .$('ux-tag-input.focus')
      .$('ux-typeahead.open')
      .$('div.ux-typeahead-options')
      .$('div')
      .$('div.ux-typeahead-loading');
    return browser.wait(EC.invisibilityOf(elem), scrollingTimeout).then(
      function () {
        return true;
      },
      function () {

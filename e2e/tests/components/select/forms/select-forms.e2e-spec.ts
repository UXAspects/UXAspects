...
describe('Select (forms) Tests', () => {
  ...
  it('should correctly validate the tag input values', async () => {
    ...
    expect(await page.confirmClassExists(page.dropdown, 'ng-invalid')).toBe(
      true,
      'Form should be invalid when the number of tags is fewer than required'
    );

    await page.clickOnCountry(true, 1);
    expect(await page.confirmClassExists(page.dropdown, 'ng-invalid')).toBe(
      false,
      'Form should be valid when the number of tags is within the correct range'
    );
    ...
    expect(await page.confirmClassExists(page.dropdown, 'ng-invalid')).toBe(
      true,
      'Form should be invalid when the number of tags is more than required'
    );

...
export class FacetContainerComponent implements OnDestroy, AfterViewInit {
  ...
  @ContentChild(FacetClearButtonDirective, { read: TemplateRef, static: false })
  ...
  constructor() {
    ...
    this.facetService.events$
      .pipe(filter<FacetDeselect>(event => event instanceof FacetDeselect))
      .subscribe(event =>
        this._announcer.announce(`Option ${event.facet.title} deselected.`, 'assertive')
      );
  }

  ...
  implements AfterContentInit, OnChanges, ControlValueAccessor, Validator, OnDestroy
{
  ...
  keyHandler(event: KeyboardEvent): void {
    ...
    const hasSelection =
      this.tagInput?.nativeElement.selectionStart !== this.tagInput?.nativeElement.selectionEnd;
  ...
  private connectTypeahead(typeahead: TypeaheadComponent): void {
    ...
    if (this.typeahead) {
      ...
      this._subscription = this.typeahead.optionSelected.subscribe(
        this.typeaheadOptionSelectedHandler.bind(this)
      );

      // Set up event handler for the highlighted element
      // Added a delay to move it out of the current change detection cycle
      this._subscription.add(
        this.typeahead.highlightedElementChange
          .pipe(tick())
...
export type TagClassFunction<T = any> = (
  tag: T,
  index: number,
  selected: boolean

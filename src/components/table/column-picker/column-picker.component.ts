...
export class ColumnPickerComponent implements OnChanges {
  ...
  @Input() selectedAriaLabel: (column: string | ColumnPickerGroupItem, index: number) => string =
    this.getDefaultSelectedAriaLabel;

  /** Define a function to get the aria label of a group in the deselected list. */
  @Input() deselectedGroupAriaLabel: (
    column: string | ColumnPickerGroupItem,
    isExpanded: boolean
  ) => string = this.getDefaultDeselectedGroupAriaLabel;

  /** Define a function that returns a column move announcement. */
  @Input() columnMovedAnnouncement: (
    column: string | ColumnPickerGroupItem,
    delta: number
  ) => string = this.getColumnMovedAnnouncement;
  ...
  private rebuildDeselectTree(): void {
    const columns = this._columnPicker.getDeselectedColumnsInPresentationOrder(
      this.deselected.slice(),
      this.sort
    );
  ...
  addColumns(
    columns: ReadonlyArray<string | ColumnPickerGroupItem> = this._deselectedSelection
  ): void {
  ...
  removeColumns(
    columns: ReadonlyArray<string | ColumnPickerGroupItem> = this._selectedSelection
  ): void {

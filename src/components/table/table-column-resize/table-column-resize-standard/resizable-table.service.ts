...
export class ResizableTableService extends BaseResizableTableService {
  ...
  resizeColumn(index: number, delta: number, isDragging: boolean = true): void {
    ...
    columns = this.setColumnWidth(
      sibling,
      Math.round(this.getColumnWidth(sibling, ColumnUnit.Pixel) - delta),
      ColumnUnit.Pixel,
      columns
    ) as number[];
    ...
    if (
      !this.isWidthValid(index, this.getColumnWidth(index, ColumnUnit.Pixel, columns)) ||
      !this.isWidthValid(sibling, this.getColumnWidth(sibling, ColumnUnit.Pixel, columns))
    ) {
      return;
    }
  ...
  getVariableColumn(delta: number): ResizableTableColumnComponent | null {
    ...
    const variableColumns = this._columns.filter(
      column => !column.isFixedWidth && !column.disabled
    );
    ...
    return variableColumns
      ...
      .find(
        column =>
          this.getColumnWidth(column.getCellIndex(), ColumnUnit.Pixel) >= column.minWidth + delta
      );
  }

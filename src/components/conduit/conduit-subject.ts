...
export class ConduitSubject {
  ...
  constructor(
    public conduit: ConduitMetadata,
    private readonly _zone: ConduitZone,
    ...
    _zone
      .getEvents()
      .pipe(
        filter(event => event.conduit.id === conduit.id),
  ...
  getInitialValue(): void {
    ...
    subjects.sort((subjectOne, subjectTwo) =>
      subjectOne.conduit.lastModified.getTime() < subjectTwo.conduit.lastModified.getTime() ? 1 : -1
    );

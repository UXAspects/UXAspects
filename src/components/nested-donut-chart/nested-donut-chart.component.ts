...
export class NestedDonutChartComponent implements OnInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._trackLayer = select<SVGElement, NestedDonutChartData>(
      this._chartElement.nativeElement
    ).append('g');

    // create the selection where we will draw the arcs
    this._arcLayer = select<SVGElement, NestedDonutChartData>(
      this._chartElement.nativeElement
    ).append('g');
  ...
  private render(): void {
    ...
    const arcTransition = transition('nestedDonutArcTransition')
      .ease(easeCubic)
    ...
    this._arcs
      ...
      .on('mousemove', event =>
        this.onArcMouseMove(pointer(event, this._chartElement.nativeElement))
      )

...
export class SliderComponent implements OnInit, AfterViewInit, DoCheck {
  ...
  @Output() valueChange: EventEmitter<SliderValue | number> = new EventEmitter<
    SliderValue | number
  >();
  ...
  snapToEnd(thumb: SliderThumb, forwards: boolean): void {
    this.setThumbValue(
      thumb,
      this.validateValue(thumb, forwards ? this.options.track.max : this.options.track.min)
    );
  }
  ...
  private updateTooltipPosition(thumb: SliderThumb): void {
    ...
    if (
      this._options.type === SliderType.Range &&
  ...
  updateThumbPosition(event: MouseEvent | TouchEvent, thumb: SliderThumb): void {
    ...
    let value =
      (this._options.track.max - this._options.track.min) * fraction + this._options.track.min;
  ...
  private updateValues(): void {
    ...
    const lowerPosition =
      ((lowerValue - this._options.track.min) /
        (this._options.track.max - this._options.track.min)) *
      100;
    const upperPosition =
      ((upperValue - this._options.track.min) /
        (this._options.track.max - this._options.track.min)) *
      100;
  ...
  private updateTicks(): void {
    ...
    const majorTicks = this.getTicks(majorOptions, SliderTickType.Major).filter(
      tick => tick.showTicks
    );
    const minorTicks = this.getTicks(minorOptions, SliderTickType.Minor).filter(
      tick => tick.showTicks
    );

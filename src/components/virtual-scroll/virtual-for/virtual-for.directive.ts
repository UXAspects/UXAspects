...
export class VirtualForDirective<T> implements OnInit, DoCheck, OnDestroy {
  ...
  constructor() {
    ...
    if (!this._virtualScroll) {
      throw new Error(
        'The "uxVirtualFor" directive requires the "uxVirtualForContainer" directive to be added to the parent element.'
      );
    }
  }
  ...
  private applyChanges(changes: IterableChanges<T>): void {
    ...
    changes.forEachOperation(
      (record: IterableChangeRecord<T>, previousIndex: number, currentIndex: number) => {
        ...
        if (previousIndex === null) {
          ...
          const view = this._viewContainerRef.detach(currentIndex) as EmbeddedViewRef<
            VirtualForOfContext<T>
          >;
          ...
          const view = this._viewContainerRef.get(previousIndex) as EmbeddedViewRef<
            VirtualForOfContext<T>
          >;
    ...
    for (let idx = 0; idx < this._viewContainerRef.length; idx++) {
      ...
      view.rootNodes.forEach((node: HTMLElement) => {
        ...
        this._renderer.setStyle(
          node,
          'transform',
          `translateY(${view.context.index * this._virtualScroll.itemSize}px`
        );
      });
  ...
  private createView(index: number): EmbeddedViewRef<VirtualForOfContext<T>> {
    ...
    return this._viewContainerRef.createEmbeddedView<VirtualForOfContext<T>>(
      this._templateRef,
      {
        $implicit: null,
        index: -1,

...
export class ColorPickerTestComponent {
  ...
  colors = this._colorNames.map(row =>
    row.map(colorName => new ColorPickerColor(colorName, this._colorService.resolve(colorName)))
  );
...
describe('Color Picker Component', () => {
  ...
  it('should not invalidate input when color InputMode changes and ChangeDetectionStrategy.OnPush is used', async () => {
    ...
    const colorFormatButton = nativeElement.querySelector<HTMLButtonElement>(
      '.ux-color-picker-input-toggle'
    );
    const colorInput = nativeElement.querySelector(

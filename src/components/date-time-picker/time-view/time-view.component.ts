...
export class TimeViewComponent implements OnInit, OnDestroy {
  ...
  constructor() {
    ...
    this.datepicker.date$
      .pipe(
        filter(date => date && this.value instanceof Date),
    ...
    if (this._isRangeMode && this._isRangeStart) {
      ...
      if (!this._rangeStart) {
        ...
        this.value.setHours(
          this._rangeStart.getHours(),
          this._rangeStart.getMinutes(),
          this._rangeStart.getSeconds()
        );
      }
    }
    ...
    if (this._isRangeMode && this._isRangeEnd) {
      ...
      if (!this._rangeEnd) {
        ...
        this.value.setHours(
          this._rangeEnd.getHours(),
          this._rangeEnd.getMinutes(),
          this._rangeEnd.getSeconds()
        );
      }
    }
  }
  ...
  incrementTimezone(): void {
    ...
    const currentZone = timezones.findIndex(
      zone => zone.name === timezone.name && zone.offset === timezone.offset
    );

    // try to get the previous zone
    this.timezoneChange.emit(
      timezones[currentZone + 1] ? timezones[currentZone + 1] : timezones[currentZone]
    );
  }

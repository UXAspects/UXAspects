...
export class DashboardComponent implements AfterViewInit, AfterContentInit, OnDestroy, OnChanges {
  ...
  @Input('aria-label') customAriaLabel: (
    widgets: DashboardWidgetComponent[],
    options: DashboardOptions
  ) => string | string = this.getDefaultAriaLabel;
  ...
  @ContentChildren(DashboardGrabHandleDirective, { descendants: true })
  ...
  constructor() {
    this.dashboardService.layout$
      .pipe(
        ...
        tap(() => (this.ariaLabel = this.getAriaLabel()))
      )
  ...
  ngAfterViewInit(): void {
    // set the initial dimensions
    this.dashboardService.setDimensions(
      this.dashboardElement.nativeElement.offsetWidth,
      this.dashboardElement.nativeElement.offsetHeight
    );
  }

...
export class DashboardGrabHandleService implements OnDestroy {
  ...
  constructor() {
    ...
    this._dashboard.layout$
      .pipe(
        ...
        filter(() => !this._dashboard.isGrabbing$.value)
      )
  ...
  setItemFocus(index: number, focusElement: boolean = true): void {
    ...
    this.getHandlesInOrder().forEach((handle, idx) =>
      idx === index ? handle.focus(focusElement) : handle.blur()
    );
  ...
  setSiblingItemFocus(widget: DashboardWidgetComponent, direction: ActionDirection): void {
    ...
    const target = this._dashboard
      ...
      .reduce(
        (handle, current) =>
          !handle || current.widget.getColumn() > handle.widget.getColumn() ? current : handle,
        null
      );
  ...
  getHandlesInOrder(handles = this._handles): DashboardGrabHandleDirective[] {
    ...
    return handles.sort(
      (handleOne, handleTwo) =>
        widgets.indexOf(handleOne.widget) - widgets.indexOf(handleTwo.widget)
    );
  }

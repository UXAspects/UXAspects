...
export class SankeyChart<T> {
  ...
  getFalloffPath(nodeLink: SankeyNodeLink<T>): string {
    ...
    const y = nodeLink.outputs.reduce(
      (bottom, output) => Math.max(bottom, output.bottomLeft[1]),
      0
    );
    const width = 20;
    const radius = 6;
  ...
  private getNodeColumns(
    nodeLinks: SankeyNodeLink<T>[] = this._nodeLinks.filter(node => node.inputs.length === 0),
    column: number = 0
  ): void {
    for (const nodeLink of nodeLinks) {
      ...
      this.getNodeColumns(
        nodeLink.outputs.map(output => this.getNodeLink(output.target)),
        column + 1
      );
    }
  }
  ...
  private getNodeHeights(): void {
    ...
    const total = groupList.reduce(
      (count, nodes) =>
        Math.max(
          count,
          nodes.reduce((accumulation, node) => accumulation + node.value, 0)
        ),
      0
    );
  ...
  private adjustNodeHeightsToFit(groupList: SankeyNodeLink<T>[][]) {
    ...
    while (largestColumn.height > this._height) {
      ...
      const fixedHeight =
        fixedNodes.length * this._minHeight + largestColumn.nodes.length * this._spacing;

      // If the unshrinkable height is greater than the available height, we can't continue
      if (fixedHeight > this._height) {
        throw new Error(
          `Cannot fit data into chart with minHeight = ${this._minHeight}px (need ${fixedHeight}px; ${this._height}px available)`
        );
      }
  ...
  private getColumnGroups(): { [key: number]: SankeyNodeLink<T>[] } {
    ...
    return this._nodeLinks.reduce<{ [key: number]: SankeyNodeLink<T>[] }>(
      (collection, nodeLink) => {
  ...
  private getLargestColumn(groupList: SankeyNodeLink<T>[][]): ColumnWithHeight<T> {
    ...
    for (const group of groupList) {
      const totalHeight =
        group.reduce((acc, node) => (acc += node.height), 0) + group.length * this._spacing;

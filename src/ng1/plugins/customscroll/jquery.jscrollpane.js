...
(function (factory) {
  if ( typeof define === 'function' && define.amd ) {
		...
		function JScrollPane(elem, s)
		{
			...
			function initialise(s)
			{
				...
				if (pane === undefined) {
					...
					paneHeight = elem.innerHeight() - (scrollMargin * 2);

					pane = $('<div class="jspPane" />').css('padding', originalPadding).append(elem.children());

					container = $('<div class="jspContainer" />').append(pane).appendTo(elem);
					...
					container.css({
						...
						'margin': scrollMargin + 'px'
					});

					elem.width(paneWidth + (scrollMargin * 2));
				...
				if(settings.showOnlyOnHover === true) {
					container.hover(
						function() {
							...
							$(this).find(".jspHorizontalBar").fadeIn();
						},
						function() {
							$(this).find(".jspVerticalBar").fadeOut();
							$(this).find(".jspHorizontalBar").fadeOut();
						}
					);					
				}	
				...
				if (!(isScrollableH || isScrollableV)) {
					...
					if (settings.showOnlyOnHover && !container.is(":hover")) {
						...
						setTimeout(function() {
							container.find('>.jspVerticalBar').fadeOut();
							container.find('>.jspHorizontalBar').fadeOut();
						}, 1000);
					}
				}
			...
			// If the element has max-height in pixels then return content height up to max-height.
			function getContentHeightByMaxHeight(margin) {
				var maxHeightCss = elem.css("max-height");
				if (maxHeightCss && maxHeightCss.indexOf("px") === maxHeightCss.length - 2) {
					var maxHeight = parseInt(maxHeightCss.slice(0, -2)) - (margin * 2);
					if (maxHeight > 0) {
						if (pane[0].scrollHeight > maxHeight) {
							return maxHeight;
						}
						else {
							return pane[0].scrollHeight;
						}
					}
				}
				return undefined;
			...
			// UX Aspects Modification - retain dom structure - allows us to stop reinitialising on scope destroy without altering dom
			function destroy(retainDom){
			...
			$.extend(
				jsp,
				{
					...
					// UX Aspects Modification - Retain Dom
					destroy: function(retainDom)
					{
							destroy(retainDom);
					}
				}
	...
	$.fn.jScrollPane.defaults = {
		...
		//UX Aspects - Updated the default value of vertical Drag min Height to 16px from 0px.

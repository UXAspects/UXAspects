...
(function() {
  ...
  sigma.canvas.edgehovers.curve =
    function(edge, source, target, context, settings) {
      ...
      if (!color)
        switch (edgeColor) {
          ...
          case 'target':
            ...
            break;
          default:
            color = hiddenEdgeColor || defaultEdgeColor;
            break;
        }
      ...
      var runTimes = (edge.selected) ? -1 : size * 10;

      for (var i = runTimes; i >= 0; i--) {

        context.lineWidth = (i > 0) ? ((i) + 1) * 2 - 1 : size;
        context.lineCap = "round";
        context.strokeStyle = (i > 0) ? settings('edgeHoverShadowColor') : settings('defaultEdgeColor');
        context.beginPath();

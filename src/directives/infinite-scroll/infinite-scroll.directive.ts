  ...
  implements OnInit, AfterContentInit, OnChanges, OnDestroy
{
  ...
  constructor() {
    this._canLoadManually = this._isLoading.pipe(
      combineLatest(
        this._isExhausted,
  ...
  ngAfterContentInit(): void {
    ...
    this._updateRequests
      .pipe(
        filterOperator(request => request.check),
        auditTime(200),
  ...
  private attachEventHandlers(): void {
    ...
    const target =
      this._scrollElement.nativeElement instanceof HTMLHtmlElement
        ? document
  ...
  private attachLoadButtonEvents(): void {
    ...
    this._loadButtonSubscriptions = this._loadButtonQuery.map(loadButton =>
      loadButton.loading.subscribe(this.loadNextPage.bind(this))
    );
  }
    ...
    if (this.needsData(request) && this.beginLoading(request)) {
      ...
      const subscription = observable.pipe(first()).subscribe(
        items => {
          ...
          if (
            request.filter === this.coerceFilter(this.filter) &&

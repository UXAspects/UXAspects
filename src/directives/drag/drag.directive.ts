...
export class DragDirective<T = unknown> implements OnDestroy {
  ...
  constructor() {
    ...
    this._drag.onDragEnd
      .pipe(
        filter(event => this._isDragging || (this.model && this.model === event.data)),
        takeUntil(this._onDestroy)
      )
  ...
  dragStart(event: MouseEvent): void {
    ...
    this._ngZone.run(() =>
      this._drag.onDragStart.next({ event, group: this.group, data: this.model })
    );
  ...
  private createScrollEventListener(): void {
    ...
    const target =
      this._scrollParent === document.documentElement ? 'document' : this._scrollParent;
  ...
  private getScrollOffsets(scrollElement: Element, event: MouseEvent): { x: number; y: number } {
    ...
    if (pointerOffsetX <= 0 && scrollElement.scrollLeft > 0) {
      ...
      pointerOffsetX >= scrollElement.clientWidth &&
    ...
    if (pointerOffsetY <= 0 && scrollElement.scrollTop > 0) {
      ...
      pointerOffsetY >= scrollElement.clientHeight &&

...
export class SelectionService<T> implements OnDestroy {
  ...
  toggle(...selections: T[]): void {
    selections.forEach(selection =>
      this.isSelected(selection) ? this.deselect(selection) : this.select(selection)
    );
  }
  ...
  setStrategy(mode: SelectionMode | SelectionStrategy<T>): void {
    ...
    if (mode instanceof SelectionStrategy) {
      ...
      switch (mode.toLowerCase().trim()) {
        ...
        default:
          throw new Error(
            `The selection mode '${mode}' does not exist. Valid modes are 'simple', 'row', or 'row-alt'.`
          );
      }
    }
  }

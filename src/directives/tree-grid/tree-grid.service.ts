...
export class TreeGridService implements OnDestroy {
  ...
  private getFlattenedTree(data: TreeGridItem[], parent?: TreeGridItem): TreeGridItem[] {
    ...
    return data.reduce((previous, item, index) => {
      ...
      const children =
        item.children && item.expanded ? this.getFlattenedTree(item.children, item) : [];
  ...
  private async getNormalizedChildren(
    response: TreeGridItem[] | Promise<TreeGridItem[]> | Observable<TreeGridItem[]>
  ): Promise<TreeGridItem[]> {

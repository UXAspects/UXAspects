# 13.0 to 13.1
* The `bootstrap` package is no longer a dependency of `@ux-aspects/ux-aspects` and is no longer required. Applications should replace their Bootstrap 3 stylesheet include (e.g. `bootstrap/dist/css/bootstrap.min.css` or `~bootstrap/less/bootstrap.less`) with `@ux-aspects/ux-aspects/styles/ux-aspects-bootstrap.css` (or the `.min.css`/`less/ux-aspects-bootstrap.less` variants), a complete drop-in replacement for the Bootstrap 3 stylesheet vendored from Bootstrap 3.4.1. Every Bootstrap 3 stylesheet module is included — any Bootstrap 3 class can still be used in application markup, and the glyphicon fonts are bundled in the package `fonts/` directory. Only Bootstrap's JavaScript is not included; UX Aspects never used it, as interactive behaviour comes from the Angular components.
